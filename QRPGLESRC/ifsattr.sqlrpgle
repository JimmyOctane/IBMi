...
// Procedure: GetIfsAttributes
// Purpose: Retrieve comprehensive attributes from an IFS document
// Parameter: ifsPath - Full path to the IFS file
// Returns: Result set with IFS attributes

ctl-opt nomain;

// Prototype for the procedure
dcl-pr GetIfsAttributes template;
  ifsPath varchar(1024) const;
end-pr;

// Procedure implementation
dcl-proc GetIfsAttributes export;
  dcl-pi GetIfsAttributes;
    ifsPath varchar(1024) const;
  end-pi;
  ...
  exec sql
    ...
    FROM TABLE(QSYS2.IFS_OBJECT_STATISTICS(
        START_PATH_NAME => :ifsPath,
        SUBTREE_DIRECTORIES => 'NO'
    ))
    WHERE PATH_NAME = :ifsPath;

end-proc;

// Alternative procedure for essential attributes only
dcl-proc GetIfsAttributesBasic export;
  dcl-pi GetIfsAttributesBasic;
    ifsPath varchar(1024) const;
  end-pi;
