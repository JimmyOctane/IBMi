     ...
     F*M ----------------------------------------------------------------------*
         ...
         /COPY Qcpysrc,AFSGDTR_CP
            ...
            Dcl-Proc AFS_GetShipmentDetailsByReference Export;
             Dcl-Pi *N char(8000);
       ...
       Dcl-S ResponseJsonIFS VARCHAR(32740);
       ...
       monitor;
        ...
        reset AFS_ReturnDS.return_error;
        if TransmitDS.HTTPResponse.StatusCode = RXS_HTTP_STATUS_OK;
          exsr parse;
          exsr cleanup;
        else;
          exsr parseError;
          exsr cleanup;
          AFS_ReturnDS.return_error = '1';
        endif;
       ...
       begsr init;
        ...
        reset AFS_API_SPECIAL_PATH;
        reset AFS_FOR_REFERENCE;
        exec sql
         select MG06LS, MG06SA, MG06DS
         into :AFS_API_BASE_PATH, :AFS_API_SPECIAL_PATH, :AFS_FOR_REFERENCE
         from OEPMGT06
        ...
        gUniqueID = UniqueID;
        AFS_ReturnDS.unique_Character = %editc(UniqueID:'X');
        gRefNumber = pRefNumber;

        if gRefNumber = *Blanks;
         RXS_JobLog( 'ERROR: No ref number provided' );
         exsr cleanup;
         return *Off;
        endif;
        ...
        // grab Unique number
        gLoginToken = AFS_GetLoginToken(UniqueID:GetUniqueIDds) ;
        // gLoginToken =
        //'eyJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6ImVhc3RfY29hc' +
        //'3RfbWV0YWxfZGlzdHJpYnV0b3JzX2xsYyIsImhhc0' +
        //'FwaUFjY2VzcyI6dHJ1ZSwianRpIjoiMDkzYTg0ZjgtZmY0NS00Y2Y' +
        //'xLWEyN2UtZmY2Njk2ZmI2NThmIiwiaWF0IjoxN' +
        //'zc4MTY3NjIzLCJleHAiOjE3NzgyMTA4MjN9.MFy2lvSOjCuJR4'+
        //'Y1ipX5IddECHHVpEbhaWw_RDZNVMQ';

        reset ConfigDS;
        ConfigDS.Username = AFS_USERNAME;
        ConfigDS.Password = AFS_PASSWORD;
        ConfigDS.BaseAPIPath = AFS_API_SPECIAL_PATH;
        ConfigDS.LogPath = AFS_LOG_PATH;
        ConfigDS.CreateLogFile = *On;
        ConfigDS.SpecialAPIPath = AFS_API_SPECIAL_PATH;
        ConfigDS.ClientID = AFS_CLIENT_ID;
        ConfigDS.ClientSecret = AFS_CLIENT_SECRET;
        ConfigDS.AccountID = 'AM00021822';
       ...
       begsr checkConfigDS;
        if ConfigDS.Username = *Blanks;
         RXS_JobLog( 'ERROR: No username provided' );
         exsr cleanup;
         return *Off;
        endif;

        if ConfigDS.Password = *Blanks;
         RXS_JobLog( 'ERROR: No password provided' );
         exsr cleanup;
         return *Off;
        endif;

        if ConfigDS.BaseAPIPath = *Blanks;
         RXS_JobLog( 'ERROR: No base API path provided' );
         exsr cleanup;
         return *Off;
        endif;

        if ConfigDS.CreateLogFile and ConfigDS.LogPath = *Blanks;
         RXS_JobLog( 'ERROR: No log path provided' );
         exsr cleanup;
         return *Off;
        endif;

        if ConfigDS.ClientID = *Blanks;
         RXS_JobLog( 'ERROR: No client ID provided' );
         exsr cleanup;
         return *Off;
        endif;

        if ConfigDS.ClientSecret = *Blanks;
         RXS_JobLog( 'ERROR: No client secret provided' );
         exsr cleanup;
         return *Off;
        endif;

        if ConfigDS.AccountID = *Blanks;
         RXS_JobLog( 'ERROR: No account ID provided' );
         exsr cleanup;
         return *Off;
        endif;
       endsr;
       ...
       begsr transmit;
        ...
        // Because this API is different and doesn't use the same
        //  hostname/URL as the other operations, we're relying
        TransmitDS.URI=%TrimR(AFS_API_SPECIAL_PATH:' /')
                      +'/smart-connect/referenceRelay';
        ...
        // If we don't send an empty Content-Type the server will
        //  just return useless empty responses to us
        TransmitDS.HeaderContentType = '';

        TransmitDS.CustomHeaderName(1) = 'endpoint';
        TransmitDS.CustomHeaderValue(1) = 'shipments';
        TransmitDS.CustomHeaderName(2) = 'client_id';
        TransmitDS.CustomHeaderValue(2) = %Trim(ConfigDS.ClientID);
        TransmitDS.CustomHeaderName(3) = 'client_secret';
        TransmitDS.CustomHeaderValue(3) = %Trim(ConfigDS.ClientSecret);
        TransmitDS.CustomHeaderName(4) = 'accountId';
        TransmitDS.CustomHeaderValue(4) = %Trim(ConfigDS.AccountID);
        TransmitDS.CustomHeaderName(5) = 'reference';
        TransmitDS.CustomHeaderValue(5) = %Trim(gRefNumber);

        if ConfigDS.CreateLogFile;
         TransmitDS.LogFile = %TrimR( ConfigDS.LogPath : ' /' )
              + '/afs_getdetailsbyrefnbr_' + %Char(UniqueID) + '.txt';
        endif;
        ...
        Exec SQL
          CALL QSYS2.IFS_WRITE('/home/afs/documents/response_'
                               || TRIM(CHAR(:UniqueID))
                               || '_'
                               || REPLACE(CHAR(CURRENT_TIMESTAMP), ':', '')
                               || '.json',
                               :ResponseJsonIFS,
                               FILE_CCSID => 1208,
                               OVERWRITE => 'REPLACE');
       ...
       //---------------------------------------------//
       //  GetShipmentDetails_JsonHandler             //
       //---------------------------------------------//
       ...
       Dcl-Proc GetShipmentDetails_JsonHandler;
        ...
        select;
          ...
          when pPath = '/data/totalWeight'
           and pType = RXS_JSON_STRING;
            monitor;
              RSP.TotalWeight = %Dec( %Trim(ParsedData) : 15 : 2 );
            on-error;
              RSP.TotalWeight = 0;
            endmon;
          ...
          when pPath = '/data/revenue'
           and pType = RXS_JSON_STRING;
            monitor;
              RSP.Revenue = %Dec( %Trim(ParsedData) : 15 : 2 );
            on-error;
              RSP.Revenue = 0;
            endmon;
          ...
          when pPath = '/data/stopsCount'
           and pType = RXS_JSON_STRING;
            monitor;
              RSP.StopsCount = %Int( %Trim(ParsedData) );
            on-error;
              RSP.StopsCount = 0;
            endmon;
          ...
          when pPath = '/data/transitDays'
           and pType = RXS_JSON_STRING;
            monitor;
              RSP.TransitDays = %Int( %Trim(ParsedData) );
            on-error;
              RSP.TransitDays = 0;
            endmon;
          ...
          when pPath = '/data/totalCube'
           and pType = RXS_JSON_STRING;
            monitor;
              RSP.TotalCube = %Dec( %Trim(ParsedData) : 15 : 2 );
            on-error;
              RSP.TotalCube = 0;
            endmon;
          ...
          when pPath = '/data/totalCost'
           and pType = RXS_JSON_STRING;
            monitor;
              RSP.TotalCost = %Dec( %Trim(ParsedData) : 15 : 2 );
            on-error;
              RSP.TotalCost = 0;
            endmon;
       ...
       //    and pType = RXS_JSON_ARRAY_END;
          ...
          when pPath = '/data/additionalServices[*]/charge'
           and pType = RXS_JSON_STRING;
            monitor;
              RSPAS.Charge = %Dec( %Trim(ParsedData) : 9 : 2 );
            on-error;
              RSPAS.Charge = 0;
            endmon;
          ...
          when pPath = '/data/additionalServices[*]/sellCharge'
           and pType = RXS_JSON_STRING;
            monitor;
              RSPAS.SellCharge = %Dec( %Trim(ParsedData) : 9 : 2 );
            on-error;
              RSPAS.SellCharge = 0;
            endmon;
