          ...
          //   ONE audit record is written to XMLORDAUD per transaction
          //   summarizing success or failure with detailed metrics.
          //
          // Parameters:
          ...
          //
          // Output:
          //   - XMLORDHDR - Order header records
          //   - XMLORDDTL - Order line item records
          //   - XMLORDAUD - Single audit log entry per transaction
          //
          // Processing:
          ...
          //   6. Write single comprehensive audit record to XMLORDAUD
          //   7. Commit transaction
          //
          // Error Handling:
          //   - All SQL errors are captured with SQLCODE and SQLSTATE
          //   - Single audit record written with STATUS='E' for errors
          //   - Single audit record written with STATUS='S' for success
          //   - Audit includes counts: headers, details, errors
          ...
          // 2026-02-17 JJF   Restructured to write only ONE audit record
          //                  per transaction with comprehensive summary
          //-----------------------------------------------------------
          ...
          if sqlCode < 0;
            ...
            exec sql
              INSERT INTO XMLORDAUD (
                ORDERNBR, TRANSMSNID, USERNAME, PROGRAMNM, IFSDOCUMENTPATH,
                STATUS, HEADER_COUNT, DETAIL_COUNT, DETAIL_ERRORS,
                ERROR_STAGE, ERR_SQLCODE, ERR_SQLSTATE, MESSAGE
              ) VALUES (
                'UNKNOWN', '', CURRENT_USER, 'NEUCOXML', :ifsDocPath,
                :processStatus, 0, 0, 0, :errorStage, :sqlErrCode,
          ...
          if sqlCode < 0;
            ...
            exec sql
              INSERT INTO XMLORDAUD (
                ...
                :hdrOrderNumber, :transmissionId, CURRENT_USER, 'NEUCOXML',
                :ifsDocPath, :processStatus, 0, 0, 0, :errorStage,
          ...
          if sqlCode < 0;
            ...
            exec sql
              INSERT INTO XMLORDAUD (
                ...
                :hdrOrderNumber, :transmissionId, CURRENT_USER, 'NEUCOXML',
                :ifsDocPath, :processStatus, :headerCount, 0, 0,
          ...
          exec sql
            INSERT INTO XMLORDAUD (
              ...
              :hdrOrderNumber, :transmissionId, CURRENT_USER, 'NEUCOXML',
              :ifsDocPath, :processStatus, :headerCount, :itemCount,
              :itemErrorCount, :errorStage, :sqlErrCode, :sqlErrState,
              :errorMessage
            );
