            ...
            // Copy member for CREATECUST procedure prototype
            ...
            *inlr = *on;
            ...
            dcl-proc ProcessCustomerXML;
              ...
              exec sql
                ...
                SELECT X.account_id, X.transaction_id, X.addressLineOne,
                       ...
                       COALESCE(X.invoiceEmail, '') AS invoiceEmail
                FROM xml_data,
                XMLTABLE (
                  XMLNAMESPACES(
              ...
              for i = 1 to rowCount;
                ...
                // Remove corrupted UTF-8 characters and apostrophes
                // from customer DBA and Legal names
                // Use REGEXP_REPLACE to remove â followed by any 2 chars
                exec sql
                 SET :c1Row.customerDbaName =
                    REGEXP_REPLACE(:c1Row.customerDbaName, 'â..', '', 1, 0);
                exec sql
                 SET :c1Row.customerLegalName =
                    REGEXP_REPLACE(:c1Row.customerLegalName, 'â..', '', 1, 0);

                // Check if record already exists for this ACCTID
                exec sql
                  SELECT GUID INTO :wGUID
                  FROM BECCUSTP
                  WHERE ACCTID = :c1Row.account_id
                ...
                if SQLCODE = 100;
                  ...
                  // Could log this if needed for audit purposes
                endif;
            ...
            dcl-proc ProcessCreditXML;
              ...
              dcl-s existingGUID char(36) inz;
              dcl-ds archiveResult likeds(returnArchiveDS);
              ...
              exec sql
                ...
                SELECT X.account_id, X.amountRequested, X.approvedLimit,
                      ...
                      COALESCE(X.personalGuaranteeCode, ''),
                      COALESCE(X.personalGuaranteeDesc, ''),
                      COALESCE(X.personalGuaranteeValue, ''),
                      COALESCE(X.personalGuaranteeDataType, '')
                FROM xml_data,
                XMLTABLE (
                  XMLNAMESPACES(
                   DEFAULT 'http://www.bectran.com/'),
                ...
                COLUMNS
                  ...
                  personalGuaranteeCode VARCHAR(50)
                    PATH 'other-info/personal-guarantee-info/description',
                  personalGuaranteeDesc VARCHAR(200)
                    PATH 'other-info/personal-guarantee-info/description',
                  personalGuaranteeValue VARCHAR(10)
                    PATH 'other-info/personal-guarantee-info/value',
                  personalGuaranteeDataType VARCHAR(50)
                    PATH 'other-info/personal-guarantee-info/value'
                ) AS X
                for read only;
              ...
              for i = 1 to rowCount;
                ...
                // Use REGEXP_REPLACE to remove â followed by any 2 chars
                exec sql
                 SET :c2Row.customerDbaName =
                   REGEXP_REPLACE(:c2Row.customerDbaName, 'â..', '', 1, 0);
                exec sql
                 SET :c2Row.customerLegalName =
                    REGEXP_REPLACE(:c2Row.customerLegalName, 'â..', '', 1, 0);

                // Check if credit record already exists for this ACCTID
                exec sql
                  SELECT GUID INTO :existingGUID
                  FROM BECCREDP
            ...
            dcl-proc DownloadBectranFiles;
              ...
              dcl-s RemotePath char(19) inz('/TN201809914404xml');
              ...
              // Retrieve production/test indicator from data area
              exec sql
                select DATA_AREA_VALUE
                into :ProdTest
                from table(qsys2.data_area_info(
                  data_area_name => 'PRODTEST',
              ...
              dow sqlcode = 0;
                ...
                endmon;
                  ...
                  commandString = 'AFRENAME OBJPATH(' + Q
                                  + %trim(remotepath) + '/inb/wip/'
                                  + %trim(FileName) + Q + ') NEWPATH(' + Q
                                  + %trim(remotepath) + '/inb/done/'
                                  + %trim(FileName) + Q + ')';
                  OutErrorDS = runIBMCommand(commandString);
            ...
            //********************************************************
            // CreateAllCustomers - Create all customers in ERP system
            //
            // Purpose: Finds all unprocessed BECCUSTP records and calls
            //  CreateCustomer for them. This runs after credit files
            //  are processed so BECCREDP records will be available
            //  if they exist.
            //
            // Parameters: None
            //
            // Returns: None
            //********************************************************
            dcl-proc CreateAllCustomers;

            // Local variables
            dcl-s wGUID char(36);
            dcl-s custCount int(10) inz(0);

            // Declare cursor to find all unprocessed customer records
            exec sql
              declare allCustCursor cursor for
               select GUID
               from BECCUSTP
               where STATUS = ' ';
            ...
            exec sql fetch allCustCursor into :wGUID;

            dow SQLCODE = 0;
              custCount += 1;
              // Create customer in ERP (BECCREDP record will be
